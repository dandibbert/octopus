...
func TestNormalizeResponsesFinishReason(t *testing.T) {
	cases := []struct {
		...
		{name: "failed carries error cause", status: "failed", err: &ResponsesError{Code: "server_error", Message: "boom"}, wantReason: "stop", wantErrMsg: "boom"},
	}
...
func TestTransformStreamFailedEventPreservesLegalFinishReason(t *testing.T) {
	o := &ResponseOutbound{}
	event := `{"type":"response.failed","response":{"status":"failed","error":{"code":"server_error","message":"upstream oops"}}}`
	...
	if resp.Error.Detail.Code != "server_error" {
		t.Fatalf("expected string error code preserved, got %+v", resp.Error)
	}
}

func TestTransformStreamFailedEventAcceptsNumericErrorCode(t *testing.T) {
	o := &ResponseOutbound{}
	event := `{"type":"response.failed","response":{"status":"failed","error":{"code":500,"message":"legacy proxy error"}}}`
	resp, err := o.TransformStream(context.Background(), []byte(event))
	if err != nil {
		t.Fatalf("TransformStream: %v", err)
	}
	...
	if resp.Error.Detail.Code != "500" || resp.Error.Detail.Message != "legacy proxy error" {
		t.Fatalf("numeric error code was not normalized: %+v", resp.Error)
	}
}
