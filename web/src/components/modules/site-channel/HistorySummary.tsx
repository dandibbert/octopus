        ...
        <div className="w-[min(24rem,calc(100vw-2rem))] space-y-3 p-3 text-left sm:p-4">
            <div className="space-y-1">
                ...
                <div className="text-2xs text-muted-foreground">
                    {model.group_name || model.group_key} · {t('history.lastRequest', { time: formatHistoryTime(model.history?.last_request_at ?? null, t) })}
                </div>
            </div>

            <div className="grid grid-cols-3 gap-2 text-xs">
                <div className="rounded-xl border border-border/60 bg-background/70 px-2 py-2">
                    <div className="text-muted-foreground">{t('history.success')}</div>
                    <div className="mt-1 font-semibold text-success">{summary.successCount}</div>
                </div>
                <div className="rounded-xl border border-border/60 bg-background/70 px-2 py-2">
                    <div className="text-muted-foreground">{t('history.failure')}</div>
                    <div className="mt-1 font-semibold text-destructive">{summary.failureCount}</div>
                </div>
                <div className="rounded-xl border border-border/60 bg-background/70 px-2 py-2">
                    <div className="text-muted-foreground">{t('history.successRate')}</div>
                            ...
                            <ComposedChart data={chartData} margin={{ top: 8, right: 8, bottom: 4, left: 4 }}>
                                ...
                                <RechartsTooltip
                                    ...
                                    content={({ active, payload }) => {
                                        ...
                                        return (
                                            <div className="rounded-lg border border-border/70 bg-popover/95 px-3 py-2 text-2xs shadow-md backdrop-blur">
                                                <div className="font-medium text-foreground">{formatBucketTime(point.time)}</div>
                                                <div className="mt-1 flex items-center gap-3 text-muted-foreground">
                                                    <span className="text-success">{t('history.tooltipSuccess', { n: point.success })}</span>
                                                    <span className="text-destructive">{t('history.tooltipFailure', { n: point.failure })}</span>
                                                </div>
                                                <div className="text-muted-foreground">
                                                    {t('history.tooltipSuccessRate', { rate: point.successRate === null ? '—' : `${point.successRate}%` })}
                                                </div>
                                            </div>
                                        );
                                    }}
